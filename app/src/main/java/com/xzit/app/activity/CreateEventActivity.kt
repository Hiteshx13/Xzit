...
class CreatEvents : BaseActivity(), View.OnClickListener {
    ...
    private var strOther: String? = null
    private var strPreference: String? = null
        ...
        when (view.id) {
            R.id.btnNext -> {
                ...
                if (profileUri == null) {
                    ...
                    map["postData[eventDurationStart]"] = getRequestBody("createEvent")
                    map["postData[eventDurationEnd]"] = getRequestBody("createEvent")
                    map["postData[eventTime]"] = getRequestBody(strEventTime)
                    map["postData[eventPrice]"] = getRequestBody("createEvent")
                    map["postData[eventDealPrice]"] = getRequestBody("createEvent")
                    map["postData[eventDealPrice]"] = getRequestBody("createEvent")
                    map["postData[eventDealDiscType]"] = getRequestBody("createEvent")
                    map["postData[eventDealDiscAmt]"] = getRequestBody("createEvent")
                    map["postData[eventLocation]"] = getRequestBody("createEvent")
                    ...
                    val requestCase = RequestBody.create(MediaType.parse("multipart/form-data"), "createEvent")
                    val clientId = RequestBody.create(MediaType.parse("multipart/form-data"), "createEvent")
                    val userId = RequestBody.create(MediaType.parse("multipart/form-data"), "createEvent")
                    // val type = RequestBody.create(MediaType.parse("multipart/form-data"), "createEvent")
                    val eventTitleName = RequestBody.create(MediaType.parse("multipart/form-data"), binding?.etTitle?.text.toString().trim())
                    val eventDescDetail = RequestBody.create(MediaType.parse("multipart/form-data"), binding?.etDescription?.text.toString().trim())

                    repository.callCreateEventImage(this, requestCase, clientId, userId, eventTitleName, eventDescDetail, body)
                }
            }
            ...
            R.id.etEventTime -> {
                showTimePickerDialog(mContext, true, object : OnTimeSelectedListener {
                    override fun onClick(str: String) {
                        ...
                        binding?.etEventTime?.setText(strTempTime)
                    }
                })
            }
    ...
    private fun getParamsRequestBody(): MultipartBody.Part? {
        val file = File("/storage/emulated/0/image1.jpg")
        val requestFile = RequestBody.create(MediaType.parse("multipart/form-data"), file)
        return MultipartBody.Part.createFormData("attachmentName[]", "image.jpg", requestFile)
    }
